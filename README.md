...
> Answer:
> client side routing makes your page a single paged application and makes the UI experience smoother by offloading all the data right away to the client instead of making a server request whenever we want to go to a new page. It trades a slower initial load time for virtually no load time in navigating the application.
