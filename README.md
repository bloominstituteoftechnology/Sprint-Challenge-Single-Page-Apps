...
> Answer:
> Client- side routing allows our single-page applications to offload some of the network traffic that would normally be created by having to re-draw the page with every event onto the user's machine. This is achieved by loading the entire site/app directly into the browser's cache, and then, when a component is requested, through clicking a button or a link, or some othe event is triggered, such as the user typing in an input element, the requested part can be loaded from the memory of the user's machine, rather than re-draw the entire site, requiring another request to the server, more bandwidth, and longer load times. With client-side routing, everything the app needs to handle events is already on the user's machine, so it can be loaded seemingly instantaneously!
...
- [x] Fetch a list of characters from the Rick and Morty API's Characters endpoint *`https://ric kandmortyapi.com/api/character/`* and render them to the screen.
...
- [x] Refactor to use as few Components as possible - while still readable to a React Dev. (Hint: research these patterns: HoC, render props, FaaC.)
